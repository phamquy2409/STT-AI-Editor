...
def create_archive_cleaner_plan(project_root: str | Path = DEFAULT_PROJECT_ROOT, open_folder: bool = True) -> dict[str, Any]:
    project_root = Path(project_root)
    out = ensure_report_dir(project_root, "archive_cleaner_plan")
    targets = [
        project_root / "exports",
        project_root / "music",
        project_root / "sfx",
        project_root / "workflow_templates",
    ]
    rows = []
    for t in targets:
        size = 0
        ...
        rows.append({"folder": str(t), "exists": t.exists(), "file_count": count, "size_gb": round(size/(1024**3), 3), "suggestion": "review_before_archive"})
    columns = ["folder", "exists", "file_count", "size_gb", "suggestion"]
    write_csv(out / "ARCHIVE_CLEANER_PLAN.csv", rows, columns)
    write_json(out / "archive_cleaner_plan.json", {"ok": True, "module": "075_archive_cleaner_plan", "rows": rows})
    (out / "ARCHIVE_CLEANER_PLAN.html").write_text(simple_html("Archive Cleaner Plan", rows, columns, "Chỉ tạo kế hoạch, không xoá file."), encoding="utf-8")
    if open_folder:
        try: os.startfile(out)
        except Exception: pass
    return {"ok": True, "output_dir": str(out), "report_dir": str(out), "folder_count": len(rows)}
