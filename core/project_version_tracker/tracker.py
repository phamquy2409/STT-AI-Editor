...
def create_project_version_tracker(project_root: str | Path = DEFAULT_PROJECT_ROOT, open_folder: bool = True) -> dict[str, Any]:
    project_root = Path(project_root)
    out = ensure_report_dir(project_root, "project_version_tracker")
    tracker_dir = project_root / "version_tracker"
    tracker_dir.mkdir(parents=True, exist_ok=True)
    entry = {
        "created_at": datetime.now().isoformat(timespec="seconds"),
        "project_root": str(project_root),
        "latest_xml": str(latest_files(project_root, ["*.xml"], 5)[0]) if latest_files(project_root, ["*.xml"], 5) else "",
        "note": "Snapshot version entry",
    }
    log_path = tracker_dir / "project_version_log.jsonl"
    ...
    with log_path.open("r", encoding="utf-8") as f:
        for i, line in enumerate(f, start=1):
            try:
                ...
                rows.append({"version": i, "created_at": data.get("created_at"), "latest_xml": data.get("latest_xml"), "note": data.get("note")})
            except Exception:
                pass
    columns = ["version", "created_at", "latest_xml", "note"]
    write_csv(out / "PROJECT_VERSION_TRACKER.csv", rows, columns)
    write_json(out / "project_version_tracker.json", {"ok": True, "module": "077_project_version_tracker", "rows": rows, "log_path": str(log_path)})
    (out / "PROJECT_VERSION_TRACKER.html").write_text(simple_html("Project Version Tracker", rows, columns, "Lưu dấu mốc version project."), encoding="utf-8")
    if open_folder:
        try: os.startfile(out)
        except Exception: pass
    return {"ok": True, "output_dir": str(out), "report_dir": str(out), "version_count": len(rows), "log_path": str(log_path)}
