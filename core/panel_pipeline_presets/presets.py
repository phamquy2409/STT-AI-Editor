...
PRESETS = [
    {"id": "prewedding_reel_60s", "label": "Prewedding Reel 60s", "intent": "prewedding_reel_60s", "preset": "vertical_1080_25p"},
    {"id": "prewedding_reel_30s", "label": "Prewedding Reel 30s", "intent": "prewedding_reel_30s", "preset": "vertical_1080_25p"},
    {"id": "prewedding_cinematic", "label": "Prewedding Cinematic", "intent": "prewedding_cinematic", "preset": "fhd_1080_25p"},
    {"id": "wedding_teaser_60s", "label": "Wedding Teaser 60s", "intent": "wedding_teaser_60s", "preset": "vertical_1080_25p"},
]

def create_panel_pipeline_presets(project_root: str | Path = DEFAULT_PROJECT_ROOT, open_folder: bool = True) -> dict[str, Any]:
    project_root = Path(project_root)
    out = ensure_report_dir(project_root, "panel_pipeline_presets")
    write_json(appdata_dir() / "panel_pipeline_presets.json", {"ok": True, "presets": PRESETS})
    write_json(out / "panel_pipeline_presets.json", {"ok": True, "module": "088_panel_pipeline_presets", "presets": PRESETS})
    write_csv(out / "PANEL_PIPELINE_PRESETS.csv", PRESETS, ["id", "label", "intent", "preset"])
    (out / "PANEL_PIPELINE_PRESETS.html").write_text(simple_html("Panel Pipeline Presets", PRESETS, ["id", "label", "intent", "preset"], "Preset để Premiere panel hiện dropdown/nút chạy nhanh."), encoding="utf-8")
    if open_folder:
        open_path(out)
    return {"ok": True, "output_dir": str(out), "report_dir": str(out), "preset_count": len(PRESETS)}
