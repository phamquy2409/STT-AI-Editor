...
CHECKLIST = [
    ("Premiere XML imported", "editor"),
    ("All source media online", "editor"),
    ("Music final replaced", "editor"),
    ("SFX final replaced/approved", "editor"),
    ("Timeline QC passed", "editor"),
    ("Color checked", "editor"),
    ("Audio level checked", "editor"),
    ("Client feedback applied", "editor"),
    ("Final export created", "editor"),
    ("Backup/handoff package created", "studio"),
]

def create_delivery_checklist(project_root: str | Path = DEFAULT_PROJECT_ROOT, open_folder: bool = True) -> dict[str, Any]:
    project_root = Path(project_root)
    out = ensure_report_dir(project_root, "delivery_checklist")
    rows = []
    for i, (task, owner) in enumerate(CHECKLIST, start=1):
        rows.append({"index": i, "task": task, "owner": owner, "status": "", "note": ""})
    columns = ["index", "task", "owner", "status", "note"]
    write_csv(out / "DELIVERY_CHECKLIST.csv", rows, columns)
    write_json(out / "delivery_checklist.json", {"ok": True, "module": "073_delivery_checklist", "rows": rows})
    (out / "DELIVERY_CHECKLIST.html").write_text(simple_html("Delivery Checklist", rows, columns, "Checklist trước khi giao clip final."), encoding="utf-8")
    if open_folder:
        try: os.startfile(out)
        except Exception: pass
    return {"ok": True, "output_dir": str(out), "report_dir": str(out), "task_count": len(rows)}
