...
PATCH_FLAG = "_stt_client_feedback_collector_patch_applied"

def apply_client_feedback_collector_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_client_feedback_collector_ui_installed", False):
        return
    try:
        window._stt_client_feedback_collector_ui_installed = True
        btn = QPushButton("Create Client Feedback Sheet")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Client Feedback Collector")
        ...
        _log(window, "Client Feedback Collector UI LOADED")
    except Exception:
        _log(window, "Client Feedback Collector PATCH ERROR")
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        result = create_client_feedback_collector(project_root=project_root, open_folder=True)
        _log(window, "")
        _log(window, "Client Feedback Collector DONE")
        _log(window, f"OK: {result.get('ok')}")
        _log(window, f"Output: {result.get('report_dir') or result.get('output_dir')}")
        if show_popup:
            QMessageBox.information(window, "Client Feedback Collector", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "Client Feedback Collector ERROR")
        _log(window, msg)
        if show_popup:
            QMessageBox.critical(window, "Client Feedback Collector Error", msg[-4000:])
