...
PATCH_FLAG = "_stt_smart_folder_organizer_patch_applied"

def apply_smart_folder_organizer_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_smart_folder_organizer_ui_installed", False):
        return
    try:
        window._stt_smart_folder_organizer_ui_installed = True
        btn = QPushButton("Create Smart Folder Organizer")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Smart Folder Organizer")
        ...
        _log(window, "Smart Folder Organizer UI LOADED")
    except Exception:
        _log(window, "Smart Folder Organizer PATCH ERROR")
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        result = create_smart_folder_organizer(project_root=project_root, open_folder=True)
        _log(window, "")
        _log(window, "Smart Folder Organizer DONE")
        _log(window, f"OK: {result.get('ok')}")
        _log(window, f"Output: {result.get('report_dir') or result.get('output_dir')}")
        if show_popup:
            QMessageBox.information(window, "Smart Folder Organizer", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "Smart Folder Organizer ERROR")
        _log(window, msg)
        if show_popup:
            QMessageBox.critical(window, "Smart Folder Organizer Error", msg[-4000:])
