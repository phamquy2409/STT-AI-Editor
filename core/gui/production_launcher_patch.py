...
PATCH_FLAG = "_stt_production_launcher_patch_applied"

def apply_production_launcher_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_production_launcher_ui_installed", False):
        return
    try:
        window._stt_production_launcher_ui_installed = True
        btn = QPushButton("Create Production Launcher")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Production Launcher")
        ...
        _log(window, "Production Launcher UI LOADED")
    except Exception:
        _log(window, "Production Launcher PATCH ERROR")
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        result = create_production_launcher(project_root=project_root, open_folder=True)
        _log(window, "")
        _log(window, "Production Launcher DONE")
        _log(window, f"OK: {result.get('ok')}")
        _log(window, f"Output: {result.get('report_dir') or result.get('output_dir')}")
        if show_popup:
            QMessageBox.information(window, "Production Launcher", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "Production Launcher ERROR")
        _log(window, msg)
        if show_popup:
            QMessageBox.critical(window, "Production Launcher Error", msg[-4000:])
