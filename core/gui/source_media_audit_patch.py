...
PATCH_FLAG = "_stt_source_media_audit_patch_applied"

def apply_source_media_audit_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_source_media_audit_ui_installed", False):
        return
    try:
        window._stt_source_media_audit_ui_installed = True
        btn = QPushButton("Audit Source Media")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Source Media Audit")
        ...
        _log(window, "Source Media Audit UI LOADED")
    except Exception:
        _log(window, "Source Media Audit PATCH ERROR")
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        result = audit_source_media(project_root=project_root, open_folder=True)
        _log(window, "")
        _log(window, "Source Media Audit DONE")
        _log(window, f"OK: {result.get('ok')}")
        _log(window, f"Output: {result.get('report_dir') or result.get('output_dir')}")
        if show_popup:
            QMessageBox.information(window, "Source Media Audit", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "Source Media Audit ERROR")
        _log(window, msg)
        if show_popup:
            QMessageBox.critical(window, "Source Media Audit Error", msg[-4000:])
