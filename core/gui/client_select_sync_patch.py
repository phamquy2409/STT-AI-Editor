...
PATCH_FLAG = "_stt_client_select_sync_patch_applied"

def apply_client_select_sync_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_client_select_sync_ui_installed", False):
        return
    try:
        window._stt_client_select_sync_ui_installed = True
        btn = QPushButton("Create Client Select Sync Plan")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Client Select Sync")
        ...
        _log(window, "Client Select Sync UI LOADED")
    except Exception:
        _log(window, "Client Select Sync PATCH ERROR")
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        result = create_client_select_sync_plan(project_root=project_root, open_folder=True)
        _log(window, "")
        _log(window, "Client Select Sync DONE")
        _log(window, f"OK: {result.get('ok')}")
        _log(window, f"Output: {result.get('report_dir') or result.get('output_dir')}")
        if show_popup:
            QMessageBox.information(window, "Client Select Sync", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "Client Select Sync ERROR")
        _log(window, msg)
        if show_popup:
            QMessageBox.critical(window, "Client Select Sync Error", msg[-4000:])
