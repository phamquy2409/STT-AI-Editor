...
PATCH_FLAG = "_stt_final_production_dashboard_patch_applied"

def apply_final_production_dashboard_patch(window_class) -> None:
    if getattr(window_class, PATCH_FLAG, False):
        return
    old_init = window_class.__init__
    def __init__(self, *args, **kwargs):
        old_init(self, *args, **kwargs)
        QTimer.singleShot(1760, lambda: _install_ui(self))
    window_class.__init__ = __init__
    setattr(window_class, PATCH_FLAG, True)

def _install_ui(window) -> None:
    if getattr(window, "_stt_final_production_dashboard_ui_installed", False):
        return
    try:
        window._stt_final_production_dashboard_ui_installed = True
        btn = QPushButton("Create Final Production Dashboard")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Final Production Dashboard")
        ...
        _log(window, "Final Production Dashboard UI LOADED")
    except Exception:
        _log(window, "Final Production Dashboard PATCH ERROR")
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        result = create_final_production_dashboard(project_root=project_root, open_folder=True)
        _log(window, "")
        _log(window, "Final Production Dashboard DONE")
        _log(window, f"OK: {result.get('ok')}")
        _log(window, f"Output: {result.get('report_dir') or result.get('output_dir')}")
        if show_popup:
            QMessageBox.information(window, "Final Production Dashboard", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "Final Production Dashboard ERROR")
        _log(window, msg)
        if show_popup:
            QMessageBox.critical(window, "Final Production Dashboard Error", msg[-4000:])
