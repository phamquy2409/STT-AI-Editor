...
PATCH_FLAG = "_stt_local_command_server_patch_applied"

def apply_local_command_server_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_local_command_server_ui_installed", False):
        return
    try:
        window._stt_local_command_server_ui_installed = True
        btn = QPushButton("Create Local Server Report")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Local Command Server")
        ...
        _log(window, "Local Command Server UI LOADED")
    except Exception:
        _log(window, "Local Command Server PATCH ERROR")
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        from core.local_command_server import create_local_command_server_report
        result = create_local_command_server_report(project_root=project_root, open_folder=True)
        _log(window, "")
        _log(window, "Local Command Server DONE")
        _log(window, f"OK: {result.get('ok')}")
        _log(window, f"Output: {result.get('report_dir') or result.get('output_dir')}")
        if show_popup:
            QMessageBox.information(window, "Local Command Server", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "Local Command Server ERROR")
        _log(window, msg)
        if show_popup:
            QMessageBox.critical(window, "Local Command Server Error", msg[-4000:])
