...
PATCH_FLAG = "_stt_music_beat_plan_patch_applied"

def apply_music_beat_plan_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_music_beat_plan_ui_installed", False): return
    try:
        window._stt_music_beat_plan_ui_installed = True
        btn = QPushButton("Create Music Beat Plan")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Music Beat Plan")
        ...
        _log(window, "MUSIC BEAT PLAN UI LOADED")
    except Exception:
        _log(window, "MUSIC BEAT PLAN PATCH ERROR"); _log(window, traceback.format_exc())
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        result = create_music_beat_plan(project_root=project_root, open_folder=True)
        _log(window, ""); _log(window, "MUSIC BEAT PLAN DONE")
        ...
        if show_popup:
            QMessageBox.information(window, "Music Beat Plan", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir") or result.get("dashboard_html")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "MUSIC BEAT PLAN ERROR"); _log(window, msg)
        if show_popup: QMessageBox.critical(window, "Music Beat Plan Error", msg[-4000:])
