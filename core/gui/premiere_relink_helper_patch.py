...
PATCH_FLAG = "_stt_premiere_relink_helper_patch_applied"

def apply_premiere_relink_helper_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_premiere_relink_helper_ui_installed", False): return
    try:
        window._stt_premiere_relink_helper_ui_installed = True
        btn = QPushButton("Create Premiere Relink Report")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Premiere Relink Helper")
        ...
        _log(window, "PREMIERE RELINK HELPER UI LOADED")
    except Exception:
        _log(window, "PREMIERE RELINK HELPER PATCH ERROR"); _log(window, traceback.format_exc())
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        result = create_premiere_relink_report(project_root=project_root, open_folder=True)
        _log(window, ""); _log(window, "PREMIERE RELINK HELPER DONE")
        ...
        if show_popup:
            QMessageBox.information(window, "Premiere Relink Helper", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir") or result.get("dashboard_html")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "PREMIERE RELINK HELPER ERROR"); _log(window, msg)
        if show_popup: QMessageBox.critical(window, "Premiere Relink Helper Error", msg[-4000:])
