...
PATCH_FLAG = "_stt_prewedding_batch_plan_patch_applied"

def apply_prewedding_batch_plan_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_prewedding_batch_plan_ui_installed", False): return
    try:
        window._stt_prewedding_batch_plan_ui_installed = True
        btn = QPushButton("Create Prewedding Batch Plan")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Prewedding Batch Plan")
        ...
        _log(window, "PREWEDDING BATCH PLAN UI LOADED")
    except Exception:
        _log(window, "PREWEDDING BATCH PLAN PATCH ERROR"); _log(window, traceback.format_exc())
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        result = create_prewedding_batch_plan(project_root=project_root, open_folder=True)
        _log(window, ""); _log(window, "PREWEDDING BATCH PLAN DONE")
        ...
        if show_popup:
            QMessageBox.information(window, "Prewedding Batch Plan", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir") or result.get("dashboard_html")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "PREWEDDING BATCH PLAN ERROR"); _log(window, msg)
        if show_popup: QMessageBox.critical(window, "Prewedding Batch Plan Error", msg[-4000:])
