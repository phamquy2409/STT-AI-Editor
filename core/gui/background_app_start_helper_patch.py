...
PATCH_FLAG = "_stt_background_app_start_helper_patch_applied"

def apply_background_app_start_helper_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_background_app_start_helper_ui_installed", False):
        return
    try:
        window._stt_background_app_start_helper_ui_installed = True
        btn = QPushButton("Create Background Start Helper")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Background App Start Helper")
        ...
        _log(window, "Background App Start Helper UI LOADED")
    except Exception:
        _log(window, "Background App Start Helper PATCH ERROR")
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        result = create_background_app_start_helper(project_root=project_root, open_folder=True)
        _log(window, "")
        _log(window, "Background App Start Helper DONE")
        _log(window, f"OK: {result.get('ok')}")
        _log(window, f"Output: {result.get('report_dir') or result.get('output_dir')}")
        if show_popup:
            QMessageBox.information(window, "Background App Start Helper", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "Background App Start Helper ERROR")
        _log(window, msg)
        if show_popup:
            QMessageBox.critical(window, "Background App Start Helper Error", msg[-4000:])
