...
PATCH_FLAG = "_stt_project_version_tracker_patch_applied"

def apply_project_version_tracker_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_project_version_tracker_ui_installed", False):
        return
    try:
        window._stt_project_version_tracker_ui_installed = True
        btn = QPushButton("Create Project Version Tracker")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Project Version Tracker")
        ...
        _log(window, "Project Version Tracker UI LOADED")
    except Exception:
        _log(window, "Project Version Tracker PATCH ERROR")
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        result = create_project_version_tracker(project_root=project_root, open_folder=True)
        _log(window, "")
        _log(window, "Project Version Tracker DONE")
        _log(window, f"OK: {result.get('ok')}")
        _log(window, f"Output: {result.get('report_dir') or result.get('output_dir')}")
        if show_popup:
            QMessageBox.information(window, "Project Version Tracker", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "Project Version Tracker ERROR")
        _log(window, msg)
        if show_popup:
            QMessageBox.critical(window, "Project Version Tracker Error", msg[-4000:])
