...
PATCH_FLAG = "_stt_master_dashboard_patch_applied"

def apply_master_dashboard_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_master_dashboard_ui_installed", False): return
    try:
        window._stt_master_dashboard_ui_installed = True
        btn = QPushButton("Create Master Dashboard")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Master Dashboard")
        ...
        _log(window, "MASTER DASHBOARD UI LOADED")
    except Exception:
        _log(window, "MASTER DASHBOARD PATCH ERROR"); _log(window, traceback.format_exc())
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        result = create_master_dashboard(project_root=project_root, open_folder=True)
        _log(window, ""); _log(window, "MASTER DASHBOARD DONE")
        ...
        if show_popup:
            QMessageBox.information(window, "Master Dashboard", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir") or result.get("dashboard_html")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "MASTER DASHBOARD ERROR"); _log(window, msg)
        if show_popup: QMessageBox.critical(window, "Master Dashboard Error", msg[-4000:])
