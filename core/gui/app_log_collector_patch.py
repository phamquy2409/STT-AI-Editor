...
PATCH_FLAG = "_stt_app_log_collector_patch_applied"

def apply_app_log_collector_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_app_log_collector_ui_installed", False):
        return
    try:
        window._stt_app_log_collector_ui_installed = True
        btn = QPushButton("Create App Log Package")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("App Log Collector")
        ...
        _log(window, "App Log Collector UI LOADED")
    except Exception:
        _log(window, "App Log Collector PATCH ERROR")
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        from core.app_log_collector import create_app_log_collector
        result = create_app_log_collector(project_root=project_root, open_folder=True)
        _log(window, "")
        _log(window, "App Log Collector DONE")
        _log(window, f"OK: {result.get('ok')}")
        _log(window, f"Output: {result.get('report_dir') or result.get('output_dir')}")
        if show_popup:
            QMessageBox.information(window, "App Log Collector", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "App Log Collector ERROR")
        _log(window, msg)
        if show_popup:
            QMessageBox.critical(window, "App Log Collector Error", msg[-4000:])
