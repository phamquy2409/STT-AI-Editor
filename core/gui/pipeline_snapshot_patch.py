...
PATCH_FLAG = "_stt_pipeline_snapshot_patch_applied"

def apply_pipeline_snapshot_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_pipeline_snapshot_ui_installed", False): return
    try:
        window._stt_pipeline_snapshot_ui_installed = True
        btn = QPushButton("Create Pipeline Snapshot")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Pipeline Snapshot")
        ...
        _log(window, "PIPELINE SNAPSHOT UI LOADED")
    except Exception:
        _log(window, "PIPELINE SNAPSHOT PATCH ERROR"); _log(window, traceback.format_exc())
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        result = create_pipeline_snapshot(project_root=project_root, open_folder=True)
        _log(window, ""); _log(window, "PIPELINE SNAPSHOT DONE")
        ...
        if show_popup:
            QMessageBox.information(window, "Pipeline Snapshot", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir") or result.get("dashboard_html")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "PIPELINE SNAPSHOT ERROR"); _log(window, msg)
        if show_popup: QMessageBox.critical(window, "Pipeline Snapshot Error", msg[-4000:])
