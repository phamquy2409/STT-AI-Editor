...
PATCH_FLAG = "_stt_sfx_placeholder_manager_patch_applied"

def apply_sfx_placeholder_manager_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_sfx_placeholder_manager_ui_installed", False):
        return
    try:
        window._stt_sfx_placeholder_manager_ui_installed = True
        btn = QPushButton("Create SFX Placeholder Manager")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("SFX Placeholder Manager")
        ...
        _log(window, "SFX Placeholder Manager UI LOADED")
    except Exception:
        _log(window, "SFX Placeholder Manager PATCH ERROR")
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        result = create_sfx_placeholder_manager(project_root=project_root, open_folder=True)
        _log(window, "")
        _log(window, "SFX Placeholder Manager DONE")
        _log(window, f"OK: {result.get('ok')}")
        _log(window, f"Output: {result.get('report_dir') or result.get('output_dir')}")
        if show_popup:
            QMessageBox.information(window, "SFX Placeholder Manager", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "SFX Placeholder Manager ERROR")
        _log(window, msg)
        if show_popup:
            QMessageBox.critical(window, "SFX Placeholder Manager Error", msg[-4000:])
