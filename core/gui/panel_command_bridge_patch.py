...
PATCH_FLAG = "_stt_panel_command_bridge_patch_applied"

def apply_panel_command_bridge_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_panel_command_bridge_ui_installed", False):
        return
    try:
        window._stt_panel_command_bridge_ui_installed = True
        btn = QPushButton("Create Panel Command Bridge")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Panel Command Bridge")
        ...
        _log(window, "Panel Command Bridge UI LOADED")
    except Exception:
        _log(window, "Panel Command Bridge PATCH ERROR")
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        result = create_panel_command_bridge(project_root=project_root, open_folder=True)
        _log(window, "")
        _log(window, "Panel Command Bridge DONE")
        _log(window, f"OK: {result.get('ok')}")
        _log(window, f"Output: {result.get('report_dir') or result.get('output_dir')}")
        if show_popup:
            QMessageBox.information(window, "Panel Command Bridge", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "Panel Command Bridge ERROR")
        _log(window, msg)
        if show_popup:
            QMessageBox.critical(window, "Panel Command Bridge Error", msg[-4000:])
