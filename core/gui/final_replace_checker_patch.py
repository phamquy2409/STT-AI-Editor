...
PATCH_FLAG = "_stt_final_replace_checker_patch_applied"

def apply_final_replace_checker_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_final_replace_checker_ui_installed", False):
        return
    try:
        window._stt_final_replace_checker_ui_installed = True
        btn = QPushButton("Check Final Replacements")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Final Replace Checker")
        ...
        _log(window, "Final Replace Checker UI LOADED")
    except Exception:
        _log(window, "Final Replace Checker PATCH ERROR")
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        result = check_final_replacements(project_root=project_root, open_folder=True)
        _log(window, "")
        _log(window, "Final Replace Checker DONE")
        _log(window, f"OK: {result.get('ok')}")
        _log(window, f"Output: {result.get('report_dir') or result.get('output_dir')}")
        if show_popup:
            QMessageBox.information(window, "Final Replace Checker", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "Final Replace Checker ERROR")
        _log(window, msg)
        if show_popup:
            QMessageBox.critical(window, "Final Replace Checker Error", msg[-4000:])
