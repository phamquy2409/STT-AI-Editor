...
PATCH_FLAG = "_stt_export_version_namer_patch_applied"

def apply_export_version_namer_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_export_version_namer_ui_installed", False):
        return
    try:
        window._stt_export_version_namer_ui_installed = True
        btn = QPushButton("Create Export Version Naming Plan")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Export Version Namer")
        ...
        _log(window, "Export Version Namer UI LOADED")
    except Exception:
        _log(window, "Export Version Namer PATCH ERROR")
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        result = create_export_version_namer(project_root=project_root, open_folder=True)
        _log(window, "")
        _log(window, "Export Version Namer DONE")
        _log(window, f"OK: {result.get('ok')}")
        _log(window, f"Output: {result.get('report_dir') or result.get('output_dir')}")
        if show_popup:
            QMessageBox.information(window, "Export Version Namer", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "Export Version Namer ERROR")
        _log(window, msg)
        if show_popup:
            QMessageBox.critical(window, "Export Version Namer Error", msg[-4000:])
