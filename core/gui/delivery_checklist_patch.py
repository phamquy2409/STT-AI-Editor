...
PATCH_FLAG = "_stt_delivery_checklist_patch_applied"

def apply_delivery_checklist_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_delivery_checklist_ui_installed", False):
        return
    try:
        window._stt_delivery_checklist_ui_installed = True
        btn = QPushButton("Create Delivery Checklist")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Delivery Checklist")
        ...
        _log(window, "Delivery Checklist UI LOADED")
    except Exception:
        _log(window, "Delivery Checklist PATCH ERROR")
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        result = create_delivery_checklist(project_root=project_root, open_folder=True)
        _log(window, "")
        _log(window, "Delivery Checklist DONE")
        _log(window, f"OK: {result.get('ok')}")
        _log(window, f"Output: {result.get('report_dir') or result.get('output_dir')}")
        if show_popup:
            QMessageBox.information(window, "Delivery Checklist", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "Delivery Checklist ERROR")
        _log(window, msg)
        if show_popup:
            QMessageBox.critical(window, "Delivery Checklist Error", msg[-4000:])
