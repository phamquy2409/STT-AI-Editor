...
PATCH_FLAG = "_stt_archive_cleaner_plan_patch_applied"

def apply_archive_cleaner_plan_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_archive_cleaner_plan_ui_installed", False):
        return
    try:
        window._stt_archive_cleaner_plan_ui_installed = True
        btn = QPushButton("Create Archive Cleaner Plan")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Archive Cleaner Plan")
        ...
        _log(window, "Archive Cleaner Plan UI LOADED")
    except Exception:
        _log(window, "Archive Cleaner Plan PATCH ERROR")
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        result = create_archive_cleaner_plan(project_root=project_root, open_folder=True)
        _log(window, "")
        _log(window, "Archive Cleaner Plan DONE")
        _log(window, f"OK: {result.get('ok')}")
        _log(window, f"Output: {result.get('report_dir') or result.get('output_dir')}")
        if show_popup:
            QMessageBox.information(window, "Archive Cleaner Plan", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "Archive Cleaner Plan ERROR")
        _log(window, msg)
        if show_popup:
            QMessageBox.critical(window, "Archive Cleaner Plan Error", msg[-4000:])
