...
PATCH_FLAG = "_stt_audio_cue_planner_patch_applied"

def apply_audio_cue_planner_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_audio_cue_planner_ui_installed", False):
        return
    try:
        window._stt_audio_cue_planner_ui_installed = True
        btn = QPushButton("Create Audio Cue Plan")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Audio Cue Planner")
        ...
        _log(window, "Audio Cue Planner UI LOADED")
    except Exception:
        _log(window, "Audio Cue Planner PATCH ERROR")
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        from core.audio_cue_planner import create_audio_cue_plan
        result = create_audio_cue_plan(project_root=project_root, open_folder=True)
        _log(window, "")
        _log(window, "Audio Cue Planner DONE")
        _log(window, f"OK: {result.get('ok')}")
        _log(window, f"Output: {result.get('report_dir') or result.get('output_dir')}")
        if show_popup:
            QMessageBox.information(window, "Audio Cue Planner", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "Audio Cue Planner ERROR")
        _log(window, msg)
        if show_popup:
            QMessageBox.critical(window, "Audio Cue Planner Error", msg[-4000:])
