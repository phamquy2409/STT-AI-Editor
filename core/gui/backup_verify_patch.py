...
PATCH_FLAG = "_stt_backup_verify_patch_applied"

def apply_backup_verify_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_backup_verify_ui_installed", False):
        return
    try:
        window._stt_backup_verify_ui_installed = True
        btn = QPushButton("Create Backup Verify Report")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Backup Verify Report")
        ...
        _log(window, "Backup Verify Report UI LOADED")
    except Exception:
        _log(window, "Backup Verify Report PATCH ERROR")
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        result = create_backup_verify_report(project_root=project_root, open_folder=True)
        _log(window, "")
        _log(window, "Backup Verify Report DONE")
        _log(window, f"OK: {result.get('ok')}")
        _log(window, f"Output: {result.get('report_dir') or result.get('output_dir')}")
        if show_popup:
            QMessageBox.information(window, "Backup Verify Report", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "Backup Verify Report ERROR")
        _log(window, msg)
        if show_popup:
            QMessageBox.critical(window, "Backup Verify Report Error", msg[-4000:])
