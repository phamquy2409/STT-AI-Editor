...
PATCH_FLAG = "_stt_workflow_templates_patch_applied"

def apply_workflow_templates_patch(window_class) -> None:
...
def _install_ui(window) -> None:
    if getattr(window, "_stt_workflow_templates_ui_installed", False): return
    try:
        window._stt_workflow_templates_ui_installed = True
        btn = QPushButton("Create Workflow Templates")
        btn.setMinimumHeight(34)
        btn.clicked.connect(lambda: _run(window, show_popup=True))
        box = QGroupBox("Workflow Templates")
        ...
        _log(window, "WORKFLOW TEMPLATES UI LOADED")
    except Exception:
        _log(window, "WORKFLOW TEMPLATES PATCH ERROR"); _log(window, traceback.format_exc())
...
def _run(window, show_popup: bool = False) -> None:
    try:
        ...
        result = create_workflow_templates(project_root=project_root, open_folder=True)
        _log(window, ""); _log(window, "WORKFLOW TEMPLATES DONE")
        ...
        if show_popup:
            QMessageBox.information(window, "Workflow Templates", "Đã chạy xong.\n\n" + str(result.get("report_dir") or result.get("output_dir") or result.get("dashboard_html")))
    except Exception:
        msg = traceback.format_exc()
        _log(window, "WORKFLOW TEMPLATES ERROR"); _log(window, msg)
        if show_popup: QMessageBox.critical(window, "Workflow Templates Error", msg[-4000:])
