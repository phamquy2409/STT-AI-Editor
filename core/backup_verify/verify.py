...
def create_backup_verify_report(project_root: str | Path = DEFAULT_PROJECT_ROOT, open_folder: bool = True) -> dict[str, Any]:
    project_root = Path(project_root)
    out = ensure_report_dir(project_root, "backup_verify_report")
    important = [
    ...
    for rel in important:
        p = project_root / rel
        rows.append({"item": rel, "exists": p.exists(), "path": str(p), "backup_status": "need_backup_check" if p.exists() else "missing"})
    columns = ["item", "exists", "path", "backup_status"]
    write_csv(out / "BACKUP_VERIFY_REPORT.csv", rows, columns)
    write_json(out / "backup_verify_report.json", {"ok": True, "module": "076_backup_verify", "rows": rows})
    (out / "BACKUP_VERIFY_REPORT.html").write_text(simple_html("Backup Verify Report", rows, columns, "Kiểm tra thứ cần backup trước khi xoá/dọn."), encoding="utf-8")
    if open_folder:
