...
def create_panel_source_folder_config(project_root: str | Path = DEFAULT_PROJECT_ROOT, source_folder: str | Path | None = None, open_folder: bool = True) -> dict[str, Any]:
    project_root = Path(project_root)
    out = ensure_report_dir(project_root, "panel_source_folder_config")
    source = Path(source_folder) if source_folder else Path(read_json(appdata_dir() / "gui_settings.json").get("source_folder", ""))
    config = {
        "ok": True,
        "module": "087_panel_source_folder",
        "project_root": str(project_root),
        "source_folder": str(source) if str(source) else "",
        "source_exists": source.exists() if str(source) else False,
        "saved_at": datetime.now().isoformat(timespec="seconds"),
    }
    write_json(appdata_dir() / "panel_source_folder_config.json", config)
    write_json(out / "panel_source_folder_config.json", config)
    rows = [{"key": k, "value": v} for k, v in config.items()]
    write_csv(out / "PANEL_SOURCE_FOLDER_CONFIG.csv", rows, ["key", "value"])
    (out / "PANEL_SOURCE_FOLDER_CONFIG.html").write_text(simple_html("Panel Source Folder Config", rows, ["key", "value"], "Config project/source folder để panel biết app đang dùng gì."), encoding="utf-8")
    if open_folder:
        open_path(out)
    return {"ok": True, "output_dir": str(out), "report_dir": str(out), "config": config}
