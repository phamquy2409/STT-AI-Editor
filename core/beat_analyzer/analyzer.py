...
def create_beat_grid(duration_sec: float, bpm: float, timebase: int) -> list[dict[str, Any]]:
    ...
    while t <= duration_sec:
        if i <= 4:
            section = "intro"
        elif t >= duration_sec * 0.82:
            section = "ending"
        elif duration_sec * 0.62 <= t <= duration_sec * 0.82:
            section = "climax"
        elif duration_sec * 0.42 <= t < duration_sec * 0.62:
            section = "build"
        else:
            section = "story"
        beats.append({
            "beat_index": i,
            "time_sec": round(t, 3),
            "frame": sec_to_frames(t, timebase),
            "bar": ((i - 1) // 4) + 1,
            "beat_in_bar": ((i - 1) % 4) + 1,
            "section": section,
            "is_downbeat": ((i - 1) % 4) == 0,
...
def create_beat_analyzer(
    ...
    **kwargs: Any,
    ...
    out = outdir(project_root, "beat_analyzer")
    sel = read_json(project_root / "stt_selected_music_v1.json") or read_json(appdata_dir() / "stt_selected_music_v1.json")
    selected = sel.get("selected") if isinstance(sel.get("selected"), dict) else {}
    if not selected:
        res = {"ok": False, "error": "NO_SELECTED_MUSIC", "message": "Run 113 first."}
        write_json(out / "beat_analyzer_error.json", res)
        if open_folder:
            open_path(out)
        return res

    music_file = Path(str(selected.get("file") or ""))
    bpm = fnum(selected.get("bpm"), infer_bpm_default(str(selected.get("moods") or "").split("|"), str(selected.get("filename") or "")))
    duration = fnum(selected.get("duration_sec"), target_seconds)
    if music_file.exists():
        duration = media_duration(music_file, fallback=duration)
    duration = min(duration, target_seconds) if target_seconds > 0 else duration

    beats = create_beat_grid(duration, bpm, timebase)
    markers = []
    for b in beats:
        if b["is_downbeat"]:
            markers.append({"name": f"BAR {b['bar']} {b['section']}", "frame": b["frame"], "time_sec": b["time_sec"], "section": b["section"]})

    data = {
        "ok": True,
        "module": "114_beat_analyzer",
        "updated_at": datetime.now().isoformat(timespec="seconds"),
        "music_file": str(music_file),
        "music_name": selected.get("filename"),
        "bpm": bpm,
    ...
    write_json(project_root / "stt_music_beat_map_v1.json", data)
    write_json(appdata_dir() / "stt_music_beat_map_v1.json", data)
    write_json(out / "stt_music_beat_map_v1.json", data)
    write_csv(out / "BEAT_MAP.csv", beats, ["beat_index","time_sec","frame","bar","beat_in_bar","section","is_downbeat"])
    write_csv(out / "PREMIERE_MUSIC_MARKERS.csv", markers, ["name","frame","time_sec","section"])
    (out / "BEAT_ANALYZER_REPORT.html").write_text(
        html_table("Beat Analyzer", beats[:200], ["beat_index","time_sec","frame","bar","beat_in_bar","section","is_downbeat"], "Beat grid tạm theo BPM/filename. Sau này có thể nâng cấp librosa."),
        encoding="utf-8",
    )
    if open_folder:
        open_path(out)
    return {"ok": True, "report_dir": str(out), "music": selected.get("filename"), "bpm": bpm, "duration_sec": round(duration,3), "beat_count": len(beats)}
