...
JSX_IMPORT_HELPER = r"""
// STT AI Editor - Auto Import Latest XML Helper
// Run inside Premiere ExtendScript.
(function(){
    alert("STT Auto Import Helper\\n\\nUse the STT panel to Refresh Latest XML, then import XML.\\nThis file documents the future auto-import bridge.");
})();
"""

def create_auto_import_helper(project_root: str | Path = DEFAULT_PROJECT_ROOT, open_folder: bool = True) -> dict[str, Any]:
    project_root = Path(project_root)
    out = ensure_report_dir(project_root, "auto_import_helper")
    pointer = update_latest_xml_pointer(project_root)
    jsx = out / "stt_auto_import_latest_xml.jsx"
    jsx.write_text(JSX_IMPORT_HELPER, encoding="utf-8")
    rows = [
        {"item": "latest_xml", "value": pointer.get("latest_xml")},
        {"item": "jsx_helper", "value": str(jsx)},
        {"item": "premiere_step", "value": "Panel > Refresh Latest XML > Import Latest XML"},
    ]
    write_csv(out / "AUTO_IMPORT_HELPER.csv", rows, ["item", "value"])
    write_json(out / "auto_import_helper.json", {"ok": True, "module": "085_auto_import_helper", "rows": rows, "pointer": pointer})
    (out / "AUTO_IMPORT_HELPER.html").write_text(simple_html("Auto Import Helper", rows, ["item", "value"], "Chuẩn bị bước auto-import sau khi pipeline chạy xong."), encoding="utf-8")
    if open_folder:
        open_path(out)
    return {"ok": True, "output_dir": str(out), "report_dir": str(out), "jsx": str(jsx), "latest_xml": pointer.get("latest_xml")}
