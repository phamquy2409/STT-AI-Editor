...
def create_panel_error_reporter(project_root: str | Path = DEFAULT_PROJECT_ROOT, open_folder: bool = True) -> dict[str, Any]:
    project_root = Path(project_root)
    out = ensure_report_dir(project_root, "panel_error_reporter")
    status = read_json(appdata_dir() / "local_command_server_status.json")
    errors = []
    if status.get("error"):
        errors.append({"source": "local_command_server_status", "error": status.get("error"), "state": status.get("state")})
    for p in [appdata_dir() / "premiere_latest_xml.json", appdata_dir() / "panel_source_folder_config.json"]:
        data = read_json(p)
        if data and data.get("ok") is False:
            errors.append({"source": str(p), "error": json.dumps(data, ensure_ascii=False), "state": "not_ok"})
    if not errors:
        errors.append({"source": "panel_error_reporter", "error": "no_error_found", "state": status.get("state", "unknown")})
    write_csv(out / "PANEL_ERROR_REPORT.csv", errors, ["source", "state", "error"])
    write_json(out / "panel_error_reporter.json", {"ok": True, "module": "089_panel_error_reporter", "errors": errors, "status": status})
    (out / "PANEL_ERROR_REPORT.html").write_text(simple_html("Panel Error Reporter", errors, ["source", "state", "error"], "Gom lỗi panel/local server để debug."), encoding="utf-8")
    zip_path = project_root / "exports" / f"panel_error_reporter_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
    ...
    if open_folder:
        open_path(out)
    return {"ok": True, "output_dir": str(out), "report_dir": str(out), "zip": str(zip_path), "error_count": len(errors)}
