...
def create_client_feedback_collector(project_root: str | Path = DEFAULT_PROJECT_ROOT, open_folder: bool = True) -> dict[str, Any]:
    project_root = Path(project_root)
    out = ensure_report_dir(project_root, "client_feedback_collector")
    timeline = load_timeline(project_root)
    rows = []
    if timeline:
        for clip in timeline:
            rows.append({
                "clip_index": clip.get("timeline_index", len(rows) + 1),
                "timeline_time": f"{clip.get('timeline_start', '')}-{clip.get('timeline_end', '')}",
        ...
        rows.append({"clip_index": 1, "timeline_time": "00:00", "role": "sample", "file": "", "client_status": "", "client_note": "", "editor_action": ""})

    columns = ["clip_index", "timeline_time", "role", "file", "client_status", "client_note", "editor_action"]
    write_csv(out / "CLIENT_FEEDBACK_TEMPLATE.csv", rows, columns)
    write_json(out / "client_feedback_collector.json", {"ok": True, "module": "071_client_feedback_collector", "rows": rows})
    (out / "CLIENT_FEEDBACK_GUIDE.txt").write_text("Gửi CSV này cho khách hoặc editor. Điền client_status: keep / remove / replace / revise.", encoding="utf-8")
    (out / "CLIENT_FEEDBACK_TEMPLATE.html").write_text(simple_html("Client Feedback Collector", rows, columns, "Mẫu ghi feedback khách theo từng clip."), encoding="utf-8")
    if open_folder:
        try: os.startfile(out)
        except Exception: pass
    return {"ok": True, "output_dir": str(out), "report_dir": str(out), "row_count": len(rows)}
