...
def sanitize(items: list[dict[str, Any]], fps: int, unknown_max_sec: float) -> list[dict[str, Any]]:
    rows = []
    current = 0
    for it in items:
        ...
        if real_frames > 0:
            ...
            # Last resort: avoid striped invalid media. Unknown files are kept short only.
...
def main() -> None:
    p = argparse.ArgumentParser(description="111 Quality Music XML V2: no stripe, quality-gated.")
    ...
    out = outdir(project, "quality_music_xml_111")

    tl = read_json(project / "stt_music_directed_quality_timeline_v2.json")
    if not tl:
        res = {"ok": False, "error": "NO_QUALITY_MUSIC_TIMELINE", "message": "Run 110 first."}
        write_json(out / "quality_music_xml_error.json", res)
        print(json.dumps(res, ensure_ascii=False, indent=2))
        return

    width, height = preset_size(a.preset)
    rows = sanitize(list(tl.get("items") or []), a.fps, a.unknown_max_sec)
    xml = build_xml(rows, a.fps, width, height, f"STT QUALITY MUSIC V2 {a.style_profile}")

    output = Path(a.output_xml) if a.output_xml else project / "stt_quality_music_v2_premiere_import.xml"
    ...
    data = {
        "ok": True,
        "module": "111_quality_music_xml_v2",
    ...
    write_json(project / "stt_quality_music_xml_v2.json", data)
    write_json(out / "stt_quality_music_xml_v2.json", data)
    write_csv(out / "QUALITY_MUSIC_XML_V2_TIMELINE.csv", rows, [
