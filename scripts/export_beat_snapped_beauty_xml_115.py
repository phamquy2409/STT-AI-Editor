...
def sanitize(items: list[dict[str, Any]], fps: int, preserve_unknown: bool) -> list[dict[str, Any]]:
    ...
    for it in items:
        ...
        real = fnum(it.get("duration_sec_media"), 0)
        if real <= 0:
            real = fnum(it.get("duration_sec"), 0)
        media_real = media_duration(path)
        if media_real > 0:
            real = media_real

        real_frames = fr(real, fps) if real > 0 else 0
        if real_frames > 0:
            src_in = min(src_in, max(0, real_frames - desired - 1))
            dur_frames = min(desired, max(1, real_frames - src_in - 1))
            file_duration = max(real_frames, src_in + dur_frames + fps)
            status = "measured_source_window"
        else:
            if preserve_unknown:
                dur_frames = desired
                file_duration = max(src_in + dur_frames + fps * 10, fps * 60 * 60)
                status = "unknown_virtual_preserved"
            else:
                dur_frames = min(desired, fps)
                file_duration = max(src_in + dur_frames + fps * 3, fps * 30)
        ...
        row.update({
            ...
            "source_out_frame": src_in + dur_frames,
            ...
            "source_in_sec": sec(src_in, fps),
            "source_out_sec": sec(src_in + dur_frames, fps),
...
def main() -> None:
    p = argparse.ArgumentParser(description="115 Export beat-snapped beauty XML.")
    ...
    out = outdir(project, "beat_snapped_beauty_xml_115")
    tl = read_json(project / "stt_beat_snapped_beauty_timeline_v1.json")
    if not tl:
        res = {"ok": False, "error": "NO_BEAUTY_BEAT_TIMELINE", "message": "Run 114 first."}
        write_json(out / "beat_beauty_xml_error.json", res)
        print(json.dumps(res, ensure_ascii=False, indent=2))
        return

    width, height = preset_size(a.preset)
    rows = sanitize(list(tl.get("items") or []), a.fps, a.preserve_unknown_duration)
    output = Path(a.output_xml) if a.output_xml else project / "stt_beat_snapped_beauty_premiere_import.xml"
    xml = build_xml(rows, a.fps, width, height, f"STT BEAT BEAUTY {a.style_profile}")
    ...
    data = {
        "ok": True,
        "module": "115_beat_snapped_beauty_xml",
    ...
    write_json(project / "stt_beat_snapped_beauty_xml_v1.json", data)
    write_json(out / "stt_beat_snapped_beauty_xml_v1.json", data)
    write_csv(out / "BEAT_SNAPPED_BEAUTY_XML_TIMELINE.csv", rows, [
        "index", "target_section", "filename", "timeline_start_sec", "duration_sec", "timeline_end_sec",
        "source_in_sec", "music_mode", "beat_snapped", "beauty_score", "beauty_class", "motion_class",
        "duration_status", "file"
    ])
    ...
    print(json.dumps({
        ...
        "fix": "115_beat_snapped_beauty_xml",
